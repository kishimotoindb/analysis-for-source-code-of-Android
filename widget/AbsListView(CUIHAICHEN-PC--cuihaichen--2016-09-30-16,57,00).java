...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
    ...
    private boolean startScrollIfNeeded(int y) {
        ...
        // scroll than a tap
		//mMotionY是在Down事件的时候记录的
        final int deltaY = y - mMotionY;
        final int distance = Math.abs(deltaY);
		//在这里判断是否是overscroll
        final boolean overscroll = mScrollY != 0;
		//系统判断用户当前操作是滑动之后，会进一步判断是在列表内滑动，还是已经滑动到两端了。
        if (overscroll || distance > mTouchSlop) {
            ...
            // steal from us
			// 决定是否拦截事件，不一定要在Down事件的时候。上面这句话说我们要开始拦截事件了，而当前
			// 方法是在onInterceptTouchEvent处理Action_Move事件的时候被调用的
			// 上面这句话证实了一旦一个View开始拦截事件，就不会再将事件传递给别的View。这个不让别人
			// 拦截的操作是决定拦截事件的View发出的，
    ...
    private void scrollIfNeeded(int y) {
        ...
        final int deltaY = rawDeltaY - mMotionCorrection;	//mMotionCorrection：How far the finger moved before we started scrolling
        int incrementalDeltaY = mLastY != Integer.MIN_VALUE ? y - mLastY : deltaY;	//mLastY：Y value from on the previous motion event (if any)
            ...
            if (y != mLastY) {
                ...
                if (motionView != null) {
                    ...
                    if (atEdge) {
						...
						/*1 @param deltaX Change in X in pixels
						 *2 @param deltaY Change in Y in pixels 相邻两次motion_event的y差值
						 *3 @param scrollX Current X scroll value in pixels before applying deltaX
						 *4 @param scrollY Current Y scroll value in pixels before applying deltaY
						 *5 @param scrollRangeX Maximum content scroll range along the X axis
						 *6 @param scrollRangeY Maximum content scroll range along the Y axis
						 *7 @param maxOverScrollX Number of pixels to overscroll by in either direction
						 *          along the X axis.
						 *8 @param maxOverScrollY Number of pixels to overscroll by in either direction
						 *          along the Y axis.
						 *9 @param isTouchEvent true if this scroll operation is the result of a touch event.
    ...
    @Override
	//任玉刚说的是一旦View决定拦截事件，onInterceptTouchEvent就不会再执行。那这里对多种事件进行了分类
	//处理，就是说当前View没有拦截事件的话，onInterceptTouchEvent还是会被执行？
	//他还认为一旦View一旦决定拦截，这个事件序列只能由他处理，那按照这句话的意思，在当前View不拦截事件
	//的情况下，当前容器的onInterceptTouchEvent执行的意义在哪？反正事件已经只能交给拦截事件的View了，
	//而且一旦View消费了Down事件，其后的一系列事件均会传给这个View，即使这个View不处理，也不会交给上
	//一级，而是最后直接返给Activity	
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        int action = ev.getAction();
        View v;

		//如果手指按到屏幕的时候，列表仍然在滑动，停止当前滑动
		//从这里可以侧面看出onInterceptTouchEvent可能只是在Action Down的时候执行，因为如果MOVE的时候仍然
		//执行，那么这里就矛盾了。但是后面对Action的switch条件又有Move，是怎么回事？
        if (mPositionScroller != null) {
            mPositionScroller.stop();
        }
...
//快速滑动和fling的区别在哪？
        ...
        switch (action & MotionEvent.ACTION_MASK) {
		//down事件能够决定是否拦截的种类：TOUCH_MODE_OVERFLING、TOUCH_MODE_OVERSCROLL、TOUCH_MODE_FLING
        case MotionEvent.ACTION_DOWN: {
			//touchMode是在onInterceptTouchEvent()方法执行之前确定的，至于具体怎么确定的，待看
            int touchMode = mTouchMode;
			//如果是OVERFLING或OVERSCROLL，就拦截事件 
            ...
            final int x = (int) ev.getX();
            final int y = (int) ev.getY();
			//双指感觉同时按到屏幕，但对于系统来说，down这个单一事件也一定能确定是哪个手指先碰到屏幕
            mActivePointerId = ev.getPointerId(0);
			
			//找到y所对应的条目。在条目数量不足以充满屏幕的时候，有可能找不到相应的条目
            int motionPosition = findMotionRow(y);
			
            if (touchMode != TOUCH_MODE_FLING && motionPosition >= 0) {
                // User clicked on an actual view (and was not stopping a fling).
                // Remember where the motion event started
                v = getChildAt(motionPosition - mFirstPosi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                tion);
                mMotionViewOriginalTop = v.getTop();
                mMotionX = x;
                mMotionY = y;
                mMotionPosition = motionPosition;
     ...
     * layout, all views in ActiveViews are demoted（降级） to ScrapViews. ScrapViews are old views that
