...
/**
 * 1.ImageView没有重写onTouchEvent方法，所以默认调用View的。
...
public class ImageView extends View {
    ...
    */
	...
	//这里先获取src图片的尺寸
	//这里的mDrawable存储的是src的图片，background是在View中保存的
        if (mDrawable == null) {
			...
			// aspect ratio 宽高比
			// 如果用户设置了根据图片调整控件的尺寸，那么在
            if (mAdjustViewBounds) {
		//即使用户选择了自动调整，如果控件的宽高是EXACTLY，也不会去根据图片缩放控件。
		//如果控件的尺寸是精确的，那么就会去正常计算宽高。
	...
	//resize之后还要分两种情况计算控件宽高，一种是不需要缩放的，aspect ratio=0；另一种是需要缩放的。
	//而且如果控件
		...
		//如果当前控件的宽高比和图片的宽高比不同时，进行缩放
            ...
            /* We are either don't want to preserve the drawables aspect ratio,
               ...
               the normal way.
            */
	//图片的宽高加上四个方向上的padding就是控件期望得到的尺寸，当然这个尺寸还要根据父容器
	//的大小进行调整，这个调整的
            w += pleft + pright;
            h += ptop + pbottom;
            
	//ImageView的宽高有三个期望值：src属性drawable图片的宽高，minWidth设置的宽高，background属性drawable图片的宽高
            w = Math.max(w, getSuggestedMinimumWidth());
            h = Math.max(h, getSuggestedMinimumHeight());
			
	//到这里之前的代码得到的是ImageView期望的宽高，并且不会去缩放ImageView期望的宽高
	//到这里，ImageView就得到了自己希望获取的大小。然后调用下面的方法，根据父容器的限制，
	//获取基本确定的最终尺寸
            widthSize = resolveSizeAndState(w, widthMeasureSpec, 0);
            heightSize = resolveSizeAndState(h, heightMeasureSpec, 0);
        }

	//将测量的结果赋值给控件的mMeasuredWidth和mMeasuredHeight。该方法中还会考虑控件bound的修饰效果，
	//然后得到控件最终的测量宽高并保存到控件中。
	//onMeasure就是根据属性去确定控件本身的尺寸，之所以这里不对bound修饰效果进行处理，感觉是为了解耦。
