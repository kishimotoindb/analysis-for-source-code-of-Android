...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    @Override
	//上层调用者就是Activity的setContentView
    public void setContentView(int layoutResID) {
		//如果mContentParent == null就去创建相应的对象给mContentParent和mDecor赋值
        if (mContentParent == null) {
			//这个方法只是创建mContentParent和mDecor这两个对象，并没有将布局加载到
			//视图中
            installDecor();
        } else {
			//如果mContentParent不为空，那么应该mDecor也不为空，因为这里并没有给mDecor重新赋值
            mContentParent.removeAllViews();
        }
		//重新创建视图对象，并将视图对象赋值给mContentParent。这里为什么mDecor不重新赋值？mContentParent
		//和mDecor到底是什么包含关系？
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public boolean superDispatchKeyShortcutEvent(KeyEvent event) {
            return mDecor.dispatchKeyShortcutEvent(event);
        }

        public boolean superDispatchTouchEvent(MotionEvent event) {
            return mDecor.dispatchTouchEvent(event);
        }

        public boolean superDispatchTrackballEvent(MotionEvent event) {
            return mDecor.dispatchTrackballEvent(event);
        }

        public boolean superDispatchGenericMotionEvent(MotionEvent event) {
            return mDecor.dispatchGenericMotionEvent(event);
        }
	...
	//调用setContentView的时候如果成员变量mContentParent==null，就调用这个方法来。但是这个方法并不是
	//只给mContentParent创建对象，同时也给成员变量mDecor创建对象
    private void installDecor() {
        //创建mDecor的对象
	if (mDecor == null) {
	//这个方法就是直接new了一个DecorView，DecorView是Window的内部类
