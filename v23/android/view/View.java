...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    /**
     * @hide Whether the TV's picture-in-picture is visible or not.
     */
    ...
    //如果他们三个的某一个位的值不等于0，那么就隐含着当前视图有一个相应的操作在等待执行中
            int mPrivateFlags;
    int mPrivateFlags2;
    int mPrivateFlags3;
     ...
     * @hide
     */
    //用来保存当前视图上一次可以获得的最大宽度和高度，如果本次测量新传过来的两个MeasureSpec值与旧的相同，那么就有可能不会
    //重新对这个View进行测量（如果这个view的其他参数没有改变，减少view tree测量的执行时间）
    int mOldWidthMeasureSpec = Integer.MIN_VALUE;
    /**
     * @hide
     */
    //用来保存当前视图上一次可以获得的最大宽度和高度
     ...
     * 当用户采用鼠标操作屏幕的时候，这个方法才有可能返回true。正常的in_touch_mode，这个方法
     * 会返回false
     *
     ...
     * @return True if the event was handled by the view, false otherwise.
     * <p>
     * 1.View的dispatchTouchEvent方法主要干了两件事：
     * 1）调用onTouch方法
     * 2）调用onTouchEvent方法
     * 2.dispatchTouchEvent()方法是在代码的最后return的，并不是实时判断onTouch和onTouchEvent的返回值然后直接return。
     ...
     * 我认为谷歌提供onTouch的目的是，如果用户需要自定义对事件的处理方式，可以在不修改View源代码的情况下，直接设置
     * ontouchListener监听器。当onTouch返回true的情况下，系统就不会再调用onTouchEvent方法，从而实现取代系统默认事件
     * 处理机制的目的。如果onTouch在onTouchEvent中调用，用户就没办法屏蔽系统在onTouchEvent提供的事件处理机制。
     * 但我感觉也没啥用，一般自定义控件肯定是重写onTouchEvent。除非希望随着控件的变化，附加一些自己的变化，在不影响
     * 控件本身事件处理机制的前提下，设置ontouchListener监听器，这样能够实现随动的效果。
     */
...
//****************************************从此开始：正经的对view进行dispatchTouchEvent**********************************
        ...
        if (onFilterTouchEventForSecurity(event)) {
            //handleScrollBarDragging鼠标拖拽滚动条，正常手机不会执行这个操作，所以这个if忽略
            if ((mViewFlags & ENABLED_MASK) == ENABLED && handleScrollBarDragging(event)) {
                result = true;
            }
            //noinspection SimplifiableIfStatement
            ListenerInfo li = mListenerInfo;
            //onTouch执行必须要view是enable的状态
            //onTouch()、onTouchEvent其实可以都同时处理事件，因为这里只有onTouch方法处理了事件，而且返回了true，系统才认为onTouch
            //处理了事件。系统不管你onTouch执行了什么，只要返回true，就认为处理了事件。
...
//*****************************************到此结束：正经的对view进行dispatchTouchEvent**********************************
        ...
        if (((viewFlags & CLICKABLE) == CLICKABLE ||
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_UP:
                     ...
                     *4.如果到UP事件的时候，prepressed仍然为真，说明用户抬起手指的时候，还没能判断出当前操作是滚动还是
                     *点击。
                     */
                    boolean prepressed = (mPrivateFlags & PFLAG_PREPRESSED) != 0;
				   /*是否是点击事件（TAP）的确是在UP事件时进行判断的，但是这个判断会不会执行，是有条件的。即如果up事件
				    *到来的时候，当前事件已经被判断为longpress，那么tap就不会执行（这里也有个前提，就是有longclicklistener）。
					*/
                    if ((mPrivateFlags & PFLAG_PRESSED) != 0 || prepressed) {
                        ...
                        /**
                         * 1.点击和焦点本来是没有关系的，系统也是当做两件事来处理。所以这里只是判断，当当前View被点击时，
                         *   如果View是Focusable的，那么就将焦点给它；如果不是，那就不给。给不给焦点并不影响click执行与否。
                         *   受影响的只是调用click的方式。如果没有焦点，就延迟执行。如果获取了焦点，...（待看）
                         * 2.在确定当前控件被点击后，将焦点给到了当前控件。
                         * 3.啥时候获取的焦点？为啥后面是在!focusTaken的时候才performClick？
                         */
                        if (isFocusable() && isFocusableInTouchMode() && !isFocused()) {
                            //该方法中没有调用performClick
                            focusTaken = requestFocus();
                        }

                        //如果在up事件到来之前没有判断出当前操作是滑动操作还是点击操作，默认按照点击操作处理
                        if (prepressed) {
                            // The button is being released before we actually
                            // showed it as pressed. 
                            // 上面这句话的意思就是说：当View位于一个滚动容器中时，系统会提供一个时间延迟，这个
                            // 延迟时间段内系统需要做的就是判断用户当前的操作是滑动还是点击。当PrePressed为真时，
                            // 说明UP事件来临的时候，系统还没有判断出操作是哪种类型，这时当然也不会将pressed设置
                            // 为真。即button已经释放了，但pressed状态还是false。所以这里将pressed设置为true。
                            // Make it show the pressedstate now (before scheduling the click) to ensure
                            // the user sees it.
                            setPressed(true, x, y);
                        }

                        /**
                         * 1.LongPress的触发是有时间阈值的，所以有可能up事件来临之前，时间阈值已经达到，PerformedLongPress已经执行。
                         *  	也可能up事件到来时，由于down和up事件间的事件间隔太短，没有达到LongPress触发所要求的时间阈值，那么此时
                         * 	PerformedLongPress还没有执行。
                         * 2.TAP就应该指的onClick的click事件。相对的应该是LONG_CLICKABLE的long press
                         * 	所执行的代码对应于setOnClickListener 和 setOnLongClickListener两个方法
                         * 3.如果是前面的情况，代码执行到up事件时已经执行了longpress，就不需要再执行TAP。
                         * 	如果是后面这种情况，应该执行TAP（即onClick），而不再执行longpress。
                         * 4.当前的if条件是：1）没执行过longPress 与 2）没有忽略Next Up事件。
                         * 	也就是说即使up事件到来之前没有执行longPress，但是如果系统设置为忽略Next Up事件，那么一样不在执行TAP。
                         * 5.系统默认tap和longpress只可以执行一个。（注意）
                         */
                        if (!mHasPerformedLongPress && !mIgnoreNextUpEvent) {
                            // This is a tap, so remove the longpress check
                            // longpress check是一个Runnable，
                            // 需要定时执行任务写个Runnable扔给消息队列，设置个状态变量来判断Runnable是否执行了。
                            // 这里longpress就是在down事件的时候将longpress需要执行的Runnable发给消息队列，然后
                            // 用状态变量判断是否执行了。如果想取消，只要在消息队列中移除Runnable即可。就省的用代
                            // 码一直判断时间是否到达，实时判断意味着循环执行判断代码，这可能极消耗性能。
                            removeLongPressCallback();
                        ...
                        //针对PrePressed的情况，因为down事件的时候，PrePressed如果被设置，那么就会
                 ...
                 * 4.PrePressed：在滚动容器中被设置，代表点击事件的判断起始时间有延迟。区别于pressed，这个是不
                 *   在滚动容器中，直接开始计时判断click和longPress中哪一个执行
                 */
                case MotionEvent.ACTION_DOWN:
                    /**
                     * 在post“long_press”对应的Runnable之前，初始化一下状态变量。但是这里即使没有设置longClickListener，
                     * 一样会初始化状态变量。
                     */
                    mHasPerformedLongPress = false;
					
					/* 当用户采用鼠标操作屏幕的时候，这个方法才有可能返回true。正常的in_touch_mode，这个方法
					 * 会返回false,所以这里正常情况下不会产生break。
					 */
                    ...
                    // 这里就是判断一下当前view是否在一个可以滚动的viewGroup中
                    ...
                    if (isInScrollingContainer) {
                        ...
                        //如果在可以滚动的ViewGroup中，click和longPress抛出自己的Runnable时机要晚一点，
                        //这个晚的时间就是getTapTimeout，180ms
                        //当checkForTap执行的时候，prepressed就会被置“0”
...
//3.**********************************Enable View 事件处理：结束*******************************************************	
    ...
    public boolean isInScrollingContainer() {
        ...
        while (p != null && p instanceof ViewGroup) {
            //判断view是否在ScrollingContainer中，取决于shouldDelayChildPressedState的返回值
     ...
     *                   states.系统默认DrawableState以外自定义的state数量
     * @return Returns an array holding the current {@link Drawable} state of
     * the view.
     * @see #mergeDrawableStates(int[], int[])
     */
    protected int[] onCreateDrawableState(int extraSpace) {
        //如果View在布局中设置了“DUPLICATE_PARENT_STATE”属性，那么这个View的DrawableState就完全跟随
        //父容器
    ...
    protected final void setMeasuredDimension(int measuredWidth, int measuredHeight) {
        ...
        // bounds which need to cover a larger area to allow other effects,
        // such as shadows and glows, to be drawn.
        // 应该不包括Padding、Margin属性，因为计算MeasureSpec的时候已经考虑了这两个因素的影响
        if (optical != isLayoutModeOptical(mParent)) {

            //Insets描述的是矩形的向心或离心移动的位移
            //mLayoutInsets: The layout insets in pixels, that is the distance in pixels between the
            //visible edges of this view its bounds.
            //估计就是opticalBounds到外圈的装饰边界之间的距离
            Insets insets = getOpticalInsets();

            //如果有bound修饰，onMeasure测到的宽高加上修饰的边距就是最终的尺寸
            //如果没有修饰，？
            int opticalWidth = insets.left + insets.right;
            int opticalHeight = insets.top + insets.bottom;

            //如果bound没有修饰效果，那View的onMeasure方法测得的就是measuredWidth就会直接赋值给mMeasuredWidth，
            //这两个值就是控件实际的宽高。
    ...
    /**
     * Utility to reconcile(协调) a desired size and state, with constraints imposed
     ...
     * {@link #MEASURED_STATE_TOO_SMALL}.
     *
     * View期望的，或者说需要的size已经测量完毕，此时需要根据父容器提供的measureSpec处理这个size，
     * 得到View最终的size。
     */
    public static int resolveSizeAndState(int size, int measureSpec, int childMeasuredState) {
        ...
        final int result;
        switch (specMode) {
            //如果父容器给了一个最大尺寸，如果控件没有超出父容器，就给View它想要的尺寸；如果View超出了父容器，就把父容器的剩余空间都给View。
            case MeasureSpec.AT_MOST:
                ...
                break;
            //只有在View自身的布局参数是Match_parent或者 xxdp的情况下，Spec才可能是EXACTLY。所以
            //这里不论View需要多大，都给specSize
            case MeasureSpec.EXACTLY:
                result = specSize;
                break;
            //如果是未指定状态，控件想要多少空间就给多少
    ...
    protected int getSuggestedMinimumHeight() {
        //mMinHeight是用户在布局文件或代码中对minimum height属性进行设置的结果。一般情况下用户都不设置
        //这个值，所以mMinHeight=0，类初始化时所赋的值。那么如果有background有图片，那么这里返回的值就
        //会是background图片的宽度(mBitmapHeight)。
        //如果背景是颜色图片(colorDrawble)，colorDrawable没有重写Drawable的getIntrinsicHeight()
        //方法，所以colorDrawable对应的mBackground.getMinimumHeight()就是"-1"；
    ...
    //主要就是将mPendingCheckForLongPress这个Runnable发给消息队列，开始计时
     ...
     * <p>
     * <p>The view will call startNestedScroll upon（当..的时候） initiating a scroll operation. In the case
     ...
     */
    //这里存储size的bits是30位，但是在resolveSizeAndDimension中或一个size_mask，mask是8位，等于如果size|mask之后，
    //存储size的位数就变成了24位。为什么？
    ...
    private final class CheckForTap implements Runnable {
        ...
        @Override
        public void run() {
            //清空PFLAG_PREPRESSED标志位
            mPrivateFlags &= ~PFLAG_PREPRESSED;
            setPressed(true, x, y);
            //如果是Long_Clickable的，就将long_click的runnable发送到消息队列，开始计时。
            //tap和longPress是同时开始抛出runnable进行计时的
    ...
    //没有setPressed就post一个Runnable去setPressed
