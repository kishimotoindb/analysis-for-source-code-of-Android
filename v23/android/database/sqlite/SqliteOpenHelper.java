...
/*
1.数据库版本仅可以升级（upgrade），降级方法（downgrade）的方法体代码就是输出错误信息。
2.如果数据库打开的时候是读写状态，但是使用过程中内存满了，怎么处理？
  答：
3.helper对象创建的数据库文件即使调用db.close()方法关闭了，还可以用helper对象重新创建，
当初创建helper的参数依然存在。调用db.close()方法关闭文件之后，重新打开文件还是重新创建
一个文件？
*/
