 ...
 * views within their parent ViewGroup.
 × 使用本类写自定义ViewGroup的时候，不需要在自定义控件中特意创建成员变量保存加进去的view，因为helper在操作的时候自己会
 × 遍历父容器中的view。
 × touch slop是针对开始没有拖到view，然后滑动手指，手指经过View上时，是否拖动View的一个判断条件。
 ×
 */
public class ViewDragHelper {
	...
	//存了x，y，如果Action Down的时候点击到了edge，edge也被记录
    private void saveInitialMotion(float x, float y, int pointerId) {
	//如果Action Down or Pointer Down的时候，pointer的数量增加了，就需要增加存储x/y数组的长度
	...
	//tryCaptureView只是capture父容器下一层的View或ViewGroup，所以这一层的ViewGroup中的View是否需要捕获，不需要写在这个方法中。传给该方法的参数toCapture就只是父容器下一层的View或ViewGroup，即使在tryCaptureView方法中写了二级及以上层次的View，也是没用的。
     ...
     */
	//某一时间点上，仅有一个手指的动作是有效的。有效手指为最后一个接触屏幕同时触摸到正在dragging view的手指。
	//当前有效的手指id存储在mActivePointerId中。
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
		//ACTION_DOWN的时候去捕获将要drag的view
		//捕获view的逻辑：
		// 1.首先根据点击事件的坐标确定位于坐标点的view
		// 2.然后判断这个view用户是否让拖动
		// 3.如果让拖动，将view赋值给成员变量，到此，表示已经捕获了view
            case MotionEvent.ACTION_DOWN: {
                final float x = ev.getX();
                final float y = ev.getY();
                final int pointerId = MotionEventCompat.getPointerId(ev, 0);
		//获取当前坐标最上方的View或ViewGroup
                final View toCapture = findTopChildUnder((int) x, (int) y);

                saveInitialMotion(x, y, pointerId);
		// 按照下面的说法，对于VewDragHelper来说，设置不设置touch slop是一样的。
                // Since the parent is already directly processing this touch event,
                // there is no reason to delay for a slop before dragging.
                // Start immediately if possible.
		// 这个方法里调用了onViewCaptured()
                tryCaptureViewForDrag(toCapture, pointerId);
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A ViewDragHelper can only manipulate one view at a time.
		// 如果需要同时拖动两个及以上的View，需要两个父容器分别调用各自的ViewDragHelper
                if (mDragState == STATE_IDLE) {
			...
			//获取所有正在触摸屏幕的手指，并执行代码
                    for (int i = 0; i < pointerCount; i++) {
                        ...
                        final float x = MotionEventCompat.getX(ev, i);
                        final float y = MotionEventCompat.getY(ev, i);
			//mInitialMotionX存储的是对应pointer按下屏幕时的坐标
			//dx,dy就是当前move相对于按下时的位移
                        final float dx = x - mInitialMotionX[pointerId];
                        final float dy = y - mInitialMotionY[pointerId];
			...
			//如果没有view被拖动，那么当手指移动的时候，系统会实时获取当前坐标位置上的View
                        final View toCapture = findTopChildUnder((int) x, (int) y);
			//当手指在view上划过时，系统会同时判断当前坐标位置上的view是否可以被拖动，以及手指滑动的距离
			//是否达到了touch slop。checkTouchSlop方法回调用getViewHorizontalGragRange()
			//通过这里可以实现手指划过View上方的时候开始拖动View的效果。
			//这里还有个问题，如果有两个手指按在屏幕上，一个在View上，一个不在，但是移动的过程中不在的手指
			//移动到了view上，这种情况要不要切换dragging的pointer，应该也是这里设置。如果默认，那么当手指
			//移动过来的时候，是不切换的。
            ...
            case MotionEventCompat.ACTION_POINTER_UP: {
                ...
                if (mDragState == STATE_DRAGGING && pointerId == mActivePointerId) {
                    ...
                    final int pointerCount = MotionEventCompat.getPointerCount(ev);
			//当当前拖动view的手指抬起的时候，如果有其他手指在View上，就切换pointer，保持对view的拖动
                    for (int i = 0; i < pointerCount; i++) {
	...
	//产生new edge的情况
	//1.当前手指按下屏幕的时候没有按到edge这个边界
	//2.要在tracing edge的范围内，即可以被跟踪
	//3.当前edge不在被锁定的范围内
	//4.当前edge没有处在拖动中
	//5.手指移动的距离大于阈值
	...
	//mCallback.onEdgeLock(edge)不重写的情况下，默认返回false，也就是说没限制任何边界
	...
	//如果getViewHorizontalDragRange返回0,那就不会去捕获新的View
	//在手指划过View的时候，getViewHorizontalDragRange方法可以对children进行不同的设置，使有的view可以在当前情况下开始
	//滑动，有的不滑动。
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        int clampedX = left;//已经+dx的位置。clamped表示夹住x，将x夹住在一个位置，x夹住的坐标
        int clampedY = top;
     ...
     */
	//对于父容器来说，top child就是父容器下一层的所有View或ViewGroup。
    public View findTopChildUnder(int x, int y) {
        final int childCount = mParentView.getChildCount();
        for (int i = childCount - 1; i >= 0; i--) {
		//这里mCallback.getOrderedChildIndex(i)默认传入i，返回i。默认的情况父容器下一层的所有View或ViewGroup
		//都会被遍历到，如果重写这个回调，就可以将某个ViewGroup或View排除在外。
