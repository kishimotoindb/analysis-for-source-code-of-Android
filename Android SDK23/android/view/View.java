    ...
    public static int resolveSizeAndState(int size, int measureSpec, int childMeasuredState) {
		...
		//如果父容器给了一个最大尺寸，如果控件没有超出父容器，就给View它想要的尺寸；如果View超出了父容器，就把父容器的剩余空间都给View。
            case MeasureSpec.AT_MOST:
                ...
                break;
		//如果父容器给的是match_parent，就把父容器目前剩余的空间全部给view，即使view希望获取的空间不用这么大
		//如果父容器给的精确大小，view就使用这个大小，不考虑view希望要多大空间，大了小了就这样。应该不会出现小了的情况，
		//因为measureSpec在获取的时候是参考view本身的布局参数和父容器的限制。
？//如果想完全看明白这里，需要知道传进来的measureSpec是怎么确定的
    ...
    protected int getSuggestedMinimumHeight() {
		//mMinWidth是用户在布局文件或代码中对minimum width属性进行设置的结果。一般情况下用户都不设置
		//这个值，所以mMinWidth=0，类初始化时所赋的值。那么如果有background有图片，那么这里返回的值就
		//会是background图片的宽度。
		
		//注意：如果背景是颜色图片，mMinWidth还是不是0？
