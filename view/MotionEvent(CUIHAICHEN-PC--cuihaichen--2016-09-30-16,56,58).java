 ...
 * screens emit one movement trace for each finger. 
 * 
 * The individual fingers or other objects that generate movement traces are referred to as <em>pointers</em>.
 * pointer就是指手指，即一个触摸屏幕的物体。如果用户用两只触摸笔同时触摸屏幕，那么一个触摸笔就是一个pointer
 *
 ...
 * with {@link #ACTION_CANCEL}.
	//Action_Down和Action_Up状态，一个完整的gesture过程只触发一次。分别对应于第一个按下的手指和最后一个抬起的手指
	//其余手指的按下和抬起对应于ACTION_POINTER_DOWN和ACTION_POINTER_UP
 * </p><p>
 ...
 * </p><p>
 * On trackball（轨迹球） devices with source class {@link InputDevice#SOURCE_CLASS_TRACKBALL},
...
public final class MotionEvent extends InputEvent implements Parcelable {
    ...
    private static native void native(GetMetaState(long nativePtr);
