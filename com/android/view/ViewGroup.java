...
public abstract class ViewGroup extends android.view.View implements ViewParent, ViewManager {
	...
	//使用bit mask的时候，严谨的做法是：
	//1.先将原标志位清空
        mGroupFlags &= ~FLAG_MASK_FOCUSABILITY;
	//2.将保存新标志位的变量的其他位清零，因为有可能其他位因为什么意外有了数值
	//3.与原标位对象做“或”操作
    ...
    @Override
    /**
     * 1.事件分发的时候，是否需要在down事件的时候就要确定谁来拦截事件？
	 *   答：是的。原因详见2457行。
	 * 2.父容器只有在容器中的子View均不处理事件的时候，才会调用自己的dispatchTouchEvent（父类View的）
	 *	 方法来处理事件（自己处理）。
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		// 当用户使用外接鼠标/traceBall等设备的时候，会首先选中焦点，然后系统合成一个motion event
		// 这里就是判断当前motion event是不是系统合成的，如果是，...
        if (ev.isTargetAccessibilityFocus() && isAccessibilityFocusedViewOrHost()) {
            ev.setTargetAccessibilityFocus(false);
        }

        boolean handled = false; //表示事件是否被处理，也是当前dispatchTouchEvent方法的返回值
		
//1s.****************************if (onFilterTouchEventForSecurity(ev))：开始***************************	
        if (onFilterTouchEventForSecurity(ev)) {
		/**
		 * 1.onFilterTouchEventForSecurity方法的详解见View类。true，分发事件。
		 * 2.当前if方法体内的执行逻辑：
		 ×	1）2197行：当Down事件来临的时候，清空之前的状态，以便进入新的循环
		 ×	2）2209行：首先判断ViewGroup是否拦截事件
		 ×  3）2275行：ViewGroup如果不拦截事件，并且事件没有被取消。那么寻找newTouchTarget
		 × 	4）
		 */
...
//1.1s.****************************Handle an initial down：开始********************************************
            if (actionMasked == MotionEvent.ACTION_DOWN) {
			   ...
			   /*
			    * 1.这里内部会将mGroupFlags的FLAG_DISALLOW_INTERCEPT位重置
				* 2.mFirstTouchTarget=null,并且清空整个touch target单链表中的target。即每一次新的事件序列刚
				*   到来的时候，系统会清空所有touch target
				*/
                resetTouchState();
            }
//1.1e.****************************Handle an initial down：结束********************************************
            // Check for interception.
            final boolean intercepted;
	 
//1.2s.***********if (actionMasked == MotionEvent.ACTION_DOWN|| mFirstTouchTarget != null)：开始*****************
		   /**
		    * 1.该if方法体的作用：
			...
			*/
            if (actionMasked == MotionEvent.ACTION_DOWN
				...
				/**	1.Down事件的发生的时候
						1.1 ViewGroup首先调用自己的onInterceptTouchEvent方法。(一定)
						1.2 onInterceptTouchEvent方法可以返回false
						1.3 即使有子View拦截事件，如果子View不请求父容器不要拦截事件，事件一样会先经过父容器的onInterceptTouchEvent
							方法
						1.4 onInterceptTouchEvent方法执行完成后，并不能够使dispatchTouchEvent方法马上结束运行，其返回的值只不过
							存储在了intercepted变量中。
						1.5 子View是否拦截事件的返回值有可能从父容器的onInterceptTouchEvent方法得到，也有可能在后面的代码中得到。
							默认ViewGroup的onInterceptTouchEvent返回false，并且不执行其他代码，所以子View不能够在ViewGroup的
							onInterceptTouchEvent中处理事件。
						1.6 当子View拦截事件的时候，需不需要也同时拦截？
						1.7 当父类拦截了事件，事件只能通过onInterceptTouchEvent方法传递给子View
						1.8 父容器onInterceptTouchEvent方法的返回值，是仅取决于父容器，还是子View也一样影响？
						1.9 父容器的onIntercept方法在子View没有请求不许拦截的情况下， 一般都是需要执行的
						1.10Down事件所有相关控件都被收到
					2.如果子View均不拦截，父容器拦截
				*/
                if (!disallowIntercept) {
/*to be continue*/  intercepted = onInterceptTouchEvent(ev);
...
//1.2e.***********if (actionMasked == MotionEvent.ACTION_DOWN|| mFirstTouchTarget != null)：结束*****************

            // If intercepted, start normal event dispatch. Also if there is already
            // a view that is handling the gesture, do normal event dispatch.
			// 如果ViewGroup拦截了事件，或者有子View拦截了事件，那么就不再考虑是否有AccessibilityFocus的View出现的情况
            if (intercepted || mFirstTouchTarget != null) {
				//清空相应的标志位FLAG_TARGET_ACCESSIBILITY_FOCUS，清空标志位后，就不用在考虑
				//AccessibilityFocus的View出现的情况。所以上面说可以开始一个正常的分发过程。
				//这个标志位就是用轨迹球等操作时，需要首先选中view，然后系统生成一个事件给它，基本现
				//在的手机也不需要考虑。
                ev.setTargetAccessibilityFocus(false);
            }
...
//1.3s.********************************************if (!canceled && !intercepted): 开始******************
            if (!canceled && !intercepted) {
			/**
			 * 1.当前if方法体仅处理Down事件，也就是说不是用来进行事件分发的，只是用来判断子View是否拦截Down事件
			 × 2.如果事件没有取消，或者没有被ViewGroup拦截，就进入if方法体，寻找newTouchTarget。
			 × 3.事件分发到ViewGroup层时，先调用ViewGroup的onInterceptTouchEvent方法判断ViewGroup是否拦截事件。如果拦截，
			 ×   ...（待续）；如果不拦截，就进入下面的if方法体内来寻找子View是否拦截事件。
			 × 4.如果说子View在父容器的onInterceptTouchEvent方法中执行了操作，使onInterceptTouchEvent返回
			 *   true，系统可能一样认为事件是被ViewGroup拦截了。但实际上不是ViewGroup拦截的，所以intercepted这个变量有可能并
			 ×   不是仅仅用来表示事件是不是被ViewGroup拦截？
			 × 5.这段代码只处理down事件,所以肯定不是不会调用子view的onTouchEvent方法。
			 */
				...
				/**
				 × 1.同一时间只有一个View可以获得焦点，所以下面for循环的时候也只是判断了一次。2317行
				 */
...
//1.3.1s.********************************if (actionMasked == MotionEvent.ACTION_DOWN:开始******************
                ...
                if (actionMasked == MotionEvent.ACTION_DOWN
                    ...
                    final int childrenCount = mChildrenCount;
					//寻找newTouchTarget
//1.3.1.1s---------------------------if (newTouchTarget == null && childrenCount != 0:开始 -------------------------
                    if (newTouchTarget == null && childrenCount != 0) {
						...
						// 默认的mChildren是按照布局的顺序排的
						// 根据用户的自定义顺序和Z轴的顺序得到了preorderedList。Z轴坐标大的索引大
						// 这个方法返回null有两种情况：1）ViewGroup只有一个子View 2）子View均没有z轴坐标
                        final ArrayList<View> preorderedList = buildTouchDispatchChildList();
                        final boolean customOrder = preorderedList == null
                                && isChildrenDrawingOrderEnabled();
                        final View[] children = mChildren;
//1.3.1.1.1s---------------------------for (int i = childrenCount - 1; i >= 0; i--):开始 -------------------------						
                        for (int i = childrenCount - 1; i >= 0; i--) {
							
							/**如果子View均没有z轴坐标，那么这里将进行自定义排序（如果要求自定义排序）
							 * 这里表面看好像要再执行一次getAndVerifyPreorderedIndex方法，其实没有，因
							 * 为如果子View均没有z轴坐标，buildTouchDispatchChildList()方法直接返回null，
						     * 并没有创建preorderedList。所以这里要再进行一次自定义排序
							 * 排序的逻辑分为两种情况：
							 * 1.子View有Z轴坐标
							 * 		1)buildTouchDispatchChildList方法中，首先按照自定义的顺序排，然后
							 *        按照Z轴坐标再排。如果这个方法已经完成了排序，customOrder就是false
							 *		  getAndVerifyPreorderedIndex方法就等于没执行，其返回值与i相同。
							 * 2.子View没有Z轴坐标
							 *		2)如果没有Z轴坐标，那么就不需要考虑Z轴坐标进行排序，所以直接在这里
							 *		  调用getAndVerifyPreorderedIndex方法直接排序。这里的排序只不过是决
							 *  	  决定一下child的索引，并不是创建preorderedList。因为目前为止已经没
							 *        有根据两个条件进行重新排序的需要了，所以preorderedList空就空吧，系
							 *        统不会再去创建，反正从mChildren里已经能够按照需要的顺序拿子View了。
						 	 */
                            final int childIndex = getAndVerifyPreorderedIndex( //这个方法用来根据用户的自定义顺序修改索引
                                    childrenCount, i, customOrder);
                            final View child = getAndVerifyPreorderedView(  	//这个方法用来拿子View，只不过判断一下从
                                    preorderedList, children, childIndex);      //preorderedList还是children里拿而已。
							...
							/**
							 × 1.如果有accessibility focus的需求，当前循环就需要执行两次。
							 × 2.感觉一般情况下不会有accessibility focus的需求。
							 × 3.找到了childWithAccessibilityFocus对应的View也没有进行任何与View相关的操作。只是重新开始循环，
							 ×   这个是什么目的？
                             */
                            if (childWithAccessibilityFocus != null) {
                                ...
                                i = childrenCount - 1;	//找到之后，重头开始再执行一次循环
                            }

							// 是否能够多手指操作由View自己决定（也不太自由），相当于一个ViewGroup里可以出现不同的情况。
							// view是否能够接收多指事件，取决于：1）是不是可见VISIBLE标志位 2）是否有动画正在执行或将要执行（即使不可见）
			                /** /important true/
							 × 1.这里的isTransformedTouchPointInView方法用来确定当前点击到ViewGroup的手指坐标，是否在当前View上，如果不在，就
							 ×   continue；如果在，就接着往下执行。所以说如果View不在点击坐标上，就得不到事件。
							 × 2.比如RelativeLayout，两个控件重叠了，会怎样？
							 * 3.下面连续的3个if的逻辑是：
							 × 		1）首先判断本次循环的child view是否在当前坐标位置上：
							 ×			如果不在，就直接进入下一次循环；如果在，判断其是否是newTouchTarget
							 ×		2）判断其是否是newTouchTarget
							 ×   		如果是，结束整个循环；如果不是，继续执行当前循环至完成，然后进入下一次循环。
							 */
/*确定View是否在坐标位置上*/ if (!canViewReceivePointerEvents(child)
                                    || !isTransformedTouchPointInView(x, y, child, null)) {
								//这句代码不知道干嘛用
                                ev.setTargetAccessibilityFocus(false);
                                continue;
							...
							/**
							 × 1.Down事件的时候，只会确定一个newTouchTarget。即使在当前坐标位置上有几个View，这里也只处理
							 ×   第一个。
							 × 2.View如果在坐标位置上，那就去拿到这个View对应的touch target。
							 × 	1）先去已有的touch target中找，达到复用的目的。
							 ×	2）如果没有，那就创建一个新的touch target。
							 × 3.Down事件时候，getTouchTarget返回的应该是null，即newTouchTarget=null
							 × 4.Pointer Down事件的时候，如果手指点击的是同一个view，getTouchTarget就应该有返回值；如果点击
							 ×   的是不同的View，就应该返回的是null
							 * 5.这里之所以找到newTouchTarget之后再往下执行代码，而是直接break，有这样一个逻辑在里面：
							 *		1）创建newTouchTarget的时候，会底层调用该child view的dispatchTouchEvent。调用的目的是为
							 *		   得到该方法的返回值，这个返回值表征了child view是否拦截事件。View因为没有onInterceptTouchEvent方法，
							 *		   所以怎么才能判断View拦截不拦截事件？答案就是由view的dispatchTouchEvent返回值决定，如
							 *		   果调用该方法返回值是true，就会创建一个新的TouchTarget对象并赋值给mFirstTouchTarget，
							 *	       返回值为true并不能直接让系统识别view拦截了事件，系统是通过mFirstTouchTarget是否为空
							 *		   来判断view是否拦截了事件。
							 *		2）所以下面这段代码的执行逻辑是：
							 *			a)首先解释一下newTouchTarget的逻辑
							 *				i)当View在某一次事件分发的过程中处理了事件，才会创建这个view的touchTarget。也就是
							 *				  说即使view在坐标位置上，如果其不处理事件，也不会创建这个View的touchTarget。
							 *			b)如果view在坐标位置上，调用getTouchTarget(child)方法，看以前是否创建过这个view的，
							 *			   touchTarget，如果创建过，说明这个view可以处理事件，那么就拿到这个View的TouchTarget，
							 *			   然后退出循环，因为已经找到了newTouchTarget，一次事件只需要一个TouchTarget。
							 *			c)如果view在坐标位置上，但是调用getTouchTarget(child)方法没有获得这个view的touchTarget，
							 *			  这时有两种情况会引发当前结果，1）点击事件从来没有发生在当前child所在坐标上 2）发生过，
							 *			  但是view不能处理事件（没有事件处理能力或不处理）。
							 *			d)当放生c的情况时，就要继续向下执行代码，判断当前child view是否能被创建为TouchTarget。
							 *			  判断child view是否处理事件，就需要底层调用view的dispatchTouchEvent方法，所以这里调用
							 *			  dispatchTouchEvent方法并不是为了事件分发，而是判断view到底能不能作为newTouchTarget。
							 *			  所以说大框架的步骤5只是为了确定newTouchTarget，并不是处理事件分发。
							 *		   
							 */
                            ...
                            if (newTouchTarget != null) {
                                ...
                                /**
                                 × 1.找到newTouchTarget就直接结束循环
                                 */
                                break;
                            }
							
							//PFLAG_CANCEL_NEXT_UP_EVENT：Indicates whether the view is temporarily detached.
                            resetCancelNextUpFlag(child);
                            
                           /**
						    * Down事件的时候决定view是不是newTouchTarget
							* 这段if代码解释了如果一个View在Down事件的时候处理了事件，事件就不会再去分发到其他View。
							* 但是要注意一个问题：这个view并不包括它的父容器，也就是说，view拦截了事件，并不影响父容
							* 器拦截不拦截事件
							*/
...
/*important*/                   alreadyDispatchedToNewTouchTarget = true;
                                break;
                            }
...
//1.3.1.1.1e---------------------------for (int i = childrenCount - 1; i >= 0; i--):结束 -------------------------                       
                        if (preorderedList != null) preorderedList.clear();
                    }
//1.3.1.1e---------------------------if (newTouchTarget == null && childrenCount != 0:结束 ---------------------
//1.3.1.2s---------------------------if (newTouchTarget == null && mFirstTouchTarget != null):开始--------------
...
//1.3.1.2e---------------------------if (newTouchTarget == null && mFirstTouchTarget != null):结束--------------
                }
//1.3.1e.********************************if (actionMasked == MotionEvent.ACTION_DOWN:结束******************
            } 
//1.3e.********************************************if (!canceled && !intercepted): 结束******************

//1.4s.***************************************Down事件以外事件的处理：开始**************************************
		   /*
		    * 1.当前1.4这部分的代码是不论事件为什么类型都会执行的
			* 2.mFirstTouchTarget=null说明当前父容器从down事件开始就拦截了事件，那么执行到这里时，就会调用父容器自己的
			×	onTouchEvent方法。
			*/
            ...
            if (mFirstTouchTarget == null) {
                ...
                while (target != null) {
                    ...
                    if (alreadyDispatchedToNewTouchTarget && target == newTouchTarget) {
					   ...
					   /* 这里能够解释有的事件执行到中途的时候，子View收到ACTION_CANCEL事件的原理：
					    * 1.父容器中途拦截了事件，所以cancelChild可以在任何时候被置“0”
						*/
                        final boolean cancelChild = resetCancelNextUpFlag(target.child)
                                || intercepted;
//extremely important-------------------------------------------------------------------------------								
/*
 * 1.这里才是整个事件序列过程中进行事件分发的地方
 * 2.子View在整个事件序列过程中随时可以选择不处理事件
 */
...
//extremely important-------------------------------------------------------------------------------
						//如果父容器中途拦截了事件，那么会在这里将mFirstTouchTarget清空（如果next=null）
						//mFirstTouchTarget=null之后，父容器的onInterceptTouchEvent方法也不会再执行
                        if (cancelChild) {
                            if (predecessor == null) {
...
//1.4e.***************************************Down事件以外事件的处理：结束********************************
//1e.****************************if (onFilterTouchEventForSecurity(ev))：结束***************************
    ...
    /**
	 * 可以通过重写这个方法改变自定义ViewGroup事件分发的顺序
     ...
     * @hide
	 * view是否能够接收多指事件，取决于：1）是不是可见 2）是否有动画正在执行或将要执行
     */
     ...
     * @hide
	 * 将ViewGroup的坐标（相对于ViewGroup的左上角）转换为View的坐标（相对于View的左上角）
     */
    ...
    /**
     * @hide
	 * 只是对坐标变换了一下坐标轴，并没有判断是不是在View内
     */
    public void transformPointToViewLocal(float[] point, View child) {
		//从这可以看出mScrollX和mScrollY就是当有ScrollView之类的控件时，在其中的View相对于屏幕滑动的位移
        point[0] += mScrollX - child.mLeft;
        point[1] += mScrollY - child.mTop;

        if (!child.hasIdentityMatrix()) {
			//如果View旋转了或者缩放了，坐标轴会变，所以这里应该是对坐标进行相应的变换
     ...
     */
	/**
	 * 主要目的：根据不同情况变换event中的参数（共两种），然后执行正常的事件分发
	 * 主要处理了两个变换，然后调用dispatchTouchEvent()
	 * 1）将Event的坐标转换到以child view左上角为坐标系的坐标
	 * 2）处理了pointer数量的问题
	 */
    private boolean dispatchTransformedTouchEvent(MotionEvent event, boolean cancel,
            View child, int desiredPointerIdBits) {
        final boolean handled;
//1.**************************************ACTION_CANCEL的处理：开始*****************************************
        // Canceling motions is a special case.  We don't need to perform any transformations
        // or filtering.  The important part is the action, not the contents.
        final int oldAction = event.getAction();
		//ACTION_CANCEL也需要子View来处理，不论处理结果如何，均不再往下执行
...
/*return*/ 	return false;
        ...
        // If the number of pointers is the same and we don't need to perform any fancy（复杂的）
        // irreversible（不可逆的） transformations, then we can reuse the motion event for this
        // dispatch as long as we are careful to revert any changes we make.
        // Otherwise we need to make a copy.
        final MotionEvent transformedEvent;
		
//2.************用户期望处理的pointer数量与当前触摸屏幕的pointer数量的判断****************************
        if (newPointerIdBits == oldPointerIdBits) {
			//hasIdentityMatrix说明当前View没有进行过变换（旋转等）
			...
			//如果进行过旋转，要通过后面的代码进行坐标变换
            transformedEvent = MotionEvent.obtain(event);
        } else {
			//3.用户期望处理的pointer数量与当前触摸屏幕的pointer数量不同，也要对参数获取的event做变换
			//Splits a motion event such that it includes only a subset of pointer ids.
            transformedEvent = event.split(newPointerIdBits);
        }
//2.************用户期望处理的pointer数量与当前触摸屏幕的pointer数量的判断****************************

//3.*******************经过变换后，调用子View的dispatchTouchEvent方法：开始***************************
	   /**
		* 变换包括：
		* 1.坐标转换为以当前View的左上角为原点
		* 2.如果view进行过变换，将坐标系也进行相应的变换	
		* 3.event仅包含desiredPointer的信息（上一步“2”的最后执行的）
		*/
...
//3.*******************经过变换后，调用子View的dispatchTouchEvent方法：结束***************************
        // Done.
        transformedEvent.recycle();
        return handled;
    }
	...
	//并没有进行排序，只是根据情况选择从preorderedList还是children中取child
    ...
    ArrayList<View> buildOrderedChildList() {
        ...
        for (int i = 0; i < childrenCount; i++) {
			...
			// Z轴坐标大的索引大
			// 即使自定义了View的顺序，也会根据Z轴坐标再排一下。也就说自定义顺序并不是最终顺序
            int insertIndex = i;
    ...
    protected void dispatchSetPressed(boolean pressed) {
        ...
        for (int i = 0; i < count; i++) {
			...
			// 子View不可点击和不可长按，才可以随父容器设置pressed
     ...
     */
	//方法的参数是viewGroup的MeasureSpec，调用这个方法的时候还没有获取view的MeasureSpec
	//与measureChildWithMargins()方法的区别只是getChildMeasureSpec(.., int padding, ..)的时候，padding参数考虑不考虑
	//margin参数。
     ...
     * same trick（把戏） so the assumption should be safe here...
     */
