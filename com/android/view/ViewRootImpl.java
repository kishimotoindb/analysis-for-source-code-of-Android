...
public final class ViewRootImpl implements ViewParent,
    ...
    final Rect mWinFrame; // frame given by window manager.  应该就是当前window的宽高
    ...
    //开始执行View的三大流程
	//performTraversals上来先测量DecorView，然后才去测量别人
	//遍历的时候不只是执行View的三大流程，还有很多为了正常显示View必须进行设置的操作。
    private void performTraversals() {
		// cache mView since it is used so much below...
        final View host = mView; //mView就是DecorView
        ...
        if (host == null || !mAdded) //一切流程开始的前提是DecorView被创建了
            return;

        mIsInTraversal = true; //因为UI线程是线程不安全的，所以要用一个变量来指明当前状态
        mWillDrawSoon = true;//（猜测）这个流程中还不能调用onDraw方法，所以这里有一个变量用来指明状态
        boolean windowSizeMayChange = false;//
        boolean newSurface = false;//surface就是一个指向图像原始缓冲区的句柄。
        boolean surfaceChanged = false;//图像缓冲区改变
		
		/**这里的lp是用于Window的，说明这个lp是DecorView的，这个window的属性是什么时候和怎么得到的？
		* 在执行三大流程前，需要确定DecorView在window中的布局参数，应该是用来确定DecorView的尺寸。
		* 这里的布局参数在获取decorView的measuredSpec时应该是可以用到的，根据这个参数未必能够马上
		* 获得DecorView的尺寸，因为DecorView极有可能会选用包裹内容。
		* 
		* 这里就有问题：
		* 1.window的尺寸在屏幕尺寸确定的前提下，是固定的么，还是window也有布局参数？
		* 2.window到底是什么？
		*/
        WindowManager.LayoutParams lp = mWindowAttributes; 

        int desiredWindowWidth; //DecorView希望Window能够提供的宽高 
        int desiredWindowHeight;//为什么在这里声明，很后面才用到？

        final View.AttachInfo attachInfo = mAttachInfo; //A set of information given to a view when it is attached to its parent
														//window.

        final int viewVisibility = getHostVisibility();//如果顶级容器DecorView都不可见，会怎么样？
        boolean viewVisibilityChanged = mViewVisibility != viewVisibility
                || mNewSurfaceNeeded;
		
		/**
		 * 用来存储改变之前的window layout参数
		 * DecorView因为是顶级View，其上侧就是window，所以这里不再使用ViewGroup.LayoutParams，而是
		...
		*/
        ...
        CompatibilityInfo compatibilityInfo = mCompatibilityInfo.get(); //检测兼容性是必须的，下面遍历View的时候，View选择哪个兼容资源，跟这里
																		//有关系，如果选哪个资源都没法确定，怎么执行三大流程。
																		//兼容性参数是在哪里和什么时候获取的？这应该是
      
	  if (compatibilityInfo.supportsScreen() == mLastInCompatMode) {    //每一次执行travesal，都会去判断前一次traversal时屏幕适配的问题
																		//屏幕的兼容性参数是会变化的，设备配置（屏幕旋转）
            params = lp;
            mFullRedrawNeeded = true;   //视图全部重绘是有状态变量的
            mLayoutRequested = true;    //布局需要重绘也有状态变量。是不是说有可能只需要View重绘时，layout并不重绘，只有在layout需要重绘时，
										//才重绘。requestLayout执行的时候，是在调用它之前就判断这个状态，还是执行它的时候检查该状态？
           
		   if (mLastInCompatMode) {    //啥意思？估计是说上一次是不是兼容模式
                params.flags &= ~WindowManager.LayoutParams.FLAG_COMPATIBLE_WINDOW; //上次是兼容模式，这次将兼容模式的标志擦除
                mLastInCompatMode = false;
            } else {
                params.flags |= WindowManager.LayoutParams.FLAG_COMPATIBLE_WINDOW; //上次不是兼容模式，这次将兼容模式的标志置1
                mLastInCompatMode = true;
            }
        }
        
        mWindowAttributesChangesFlag = 0; //记录本次traversal时window属性改变，window到底是什么？
        
		//记录Display四个顶点坐标的Rect对象，估计因为调用方法去获取屏幕宽高也是比较消耗性能的，所以
		//当取到以后，就存入一个Rect对象（mWinFrame），方便下次使用。并且可以通过这个对象记录上一次
		//屏幕的宽高，然后和当前屏幕的宽高mWidth和mHeight进行比较，从而判断需不需要对上层的某些或某
		//个东西进行重新处理（感觉是三大流程）
        ...
        if (mFirst) {	// true for the first time the view is added
            ...
            mLayoutRequested = true;   	//布局也是

	   	 //判断DecorView的布局参数，根据DecorView的布局参数中的类型参数，决定DecorView所在window的宽高
	    	//确定了window的宽高，最上层的尺寸就有了。
	    	//这两个尺寸有什么不同？
            if (lp.type == WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL) {	//Window type: panel that slides out from over the status bar
         									//In multiuser systems shows on all users' windows.
                // NOTE -- system code, won't try to do compat mode.
		//获取屏幕的宽高作为DecorView所在window的宽高，进而在计算DecorView的宽高（getChildMeasure()）时，提供widthMeasuredSpec
                Point size = new Point();
				//这里拿到的只是display的宽高，不一定是手机屏幕的宽高
                mDisplay.getRealSize(size);//Gets the real size of the display without subtracting any window decor or applying any compatibility scale factors.
                //DecorView在计算尺寸的时候，是以display作为其容器宽高来计算的
				//这里就获取了最初计算尺寸的父容器宽高，而且这个值是固定的。
				desiredWindowWidth = size.x;
                desiredWindowHeight = size.y;
            } else {
				直接从mView（即DecorView本身来获取父容器宽高）
				//A structure describing general information about a display, such as its size, density, and font scaling.
                DisplayMetrics packageMetrics = 
                    mView.getContext().getResources().getDisplayMetrics();
                desiredWindowWidth = packageMetrics.widthPixels; //The absolute width of the display in pixels.
                desiredWindowHeight = packageMetrics.heightPixels;
            }

	    // attachInfo是每一个View都需要一个，还是DecorView自己有就行？
			...
			//horizontal layout direction flags for mapping attribute layoutDirection to correct flag value.
			//设置DecorView水平方向的类型，LTR，RTL等
            if (mViewLayoutDirectionInitial == View.LAYOUT_DIRECTION_INHERIT) {
                host.setLayoutDirection(mLastConfiguration.getLayoutDirection());
            }
			//为什么要将自己的attchInfo发送给window?
            host.dispatchAttachedToWindow(attachInfo, 0); //将DecorView的attachInfo信息发送给window
			//
			...
			//从frame取得父容器宽高，frame应该是边框的意思。但是
            desiredWindowWidth = frame.width();
            desiredWindowHeight = frame.height();
			//如果当前的屏幕宽高和上一次记录的屏幕宽高不同，修改状态标识，以通知系统重新执行三大流程。
		...
		//至此，DecorView的父容器宽高就已经获取到了。这个宽高应该是确定的。
	
		//当DecorView和其父容器的可见性改变时，执行一些操作，待看
        ...
        // the thread.
		// 执行本次performTraversal()方法执行之前，View需要处理的所有任务。
        getRunQueue().executeActions(attachInfo.mHandler);
		
		//记录DecorView的top，left，right，bottom坐标是否改变的变量
        boolean insetsChanged = false;
		//如果任何操作调用了requestLayout()方法，并且当前window的owner处于活动状态，这里就赋值表明需要
		//requestLayout。window的owner处于活动状态的意思应该是activity、fragment处于onStart状态（待确定）。
        ...
        if (layoutRequested) {
			...
			//重绘也需要区分是不是View的第一次显示
            if (mFirst) {
                // make sure touch mode code executes by setting cached value
                // to opposite of the added touch mode.
				// touch mode的概念就是当前的操作是由用户触摸屏幕触发的，因为安卓还有键盘和轨迹球
				//（最早的安卓手机，htc one还是什么，屏幕下方有个轨迹球来操作屏幕上的光标）。
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            if (!mStopped) { //如果window的owner是活动的状态
                boolean focusChangedDueToTouchMode = ensureTouchModeLocally(
                        (relayoutResult&WindowManagerGlobal.RELAYOUT_RES_IN_TOUCH_MODE) != 0);
				//mWidth和mHeight是从mWinFrame得到的，应该是当前window的宽高
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                        || mHeight != host.getMeasuredHeight() || contentInsetsChanged) {
					//这里的child指的是DecorView,所以方法获取的是DecorView的尺寸。
					//这里需要说明的是，当DecorView的layout_width属性是“10dp”这种精确的数字时，即使这个数字很大，
					//超出了屏幕的宽高，系统也会进行处理使宽高最大为屏幕尺寸（具体的处理位置还不清楚），但可以确
					...
					//因为draw步骤的时候，是由大到小绘制view，所以在draw之前，一定是确定了所有控件大小的。
                     // Ask host how big it wants to be
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);//将DecorView的尺寸给子元素，让他们去测量自己的尺寸
